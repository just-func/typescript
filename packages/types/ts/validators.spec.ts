...
const notJustValue = [undefined, null, 1, true, '', 'a', {}, Symbol(), () => {}, () => {}]

describe(`${isJustEmpty.name}()`, () => {
	it('returns false for not JustValue', () => {
		notJustValue.forEach((value) => {
			expect(isJustEmpty(value)).toBe(false)
		})
	})
...
describe(`${isJustUno.name}()`, () => {
	it('returns false for not JustValue', () => {
		notJustValue.forEach((value) => {
			expect(isJustUno(value)).toBe(false)
		})
	})
	...
	it('returns true for JustUno', () => {
		notJustValue.forEach((value) => {
			expect(isJustUno([value])).toBe(true)
		})
	})
...
describe(`${isJustDuo.name}()`, () => {
	it('returns false for not JustValue', () => {
		notJustValue.forEach((value) => {
			expect(isJustDuo(value)).toBe(false)
		})
	})
	...
	it('returns true for JustDuo', () => {
		notJustValue.forEach((value) => {
			expect(isJustDuo([value, {}])).toBe(true)
		})
	})
...
describe(`${isJustValue.name}()`, () => {
	it('returns false for not JustValue', () => {
		notJustValue.forEach((value) => {
			expect(isJustValue(value)).toBe(false)
		})
	})
